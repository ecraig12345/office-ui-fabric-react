...
export const prettierExtensions = ['ts', 'tsx', 'js', 'jsx', 'json', 'scss', 'html', 'md'];
 ...
 *
 * @param files - List of files for which to run prettier
 * @param configPath - Path to relevant prettier.config.js.
 * @param runAsync - Whether to run the command synchronously or asynchronously
 * @param logErrorsOnly - If true, log errors/warnings only. Otherwise log all output.
 * @returns A promise if run asynchronously, or nothing if run synchronously
 */
function runPrettier(files: string[], configPath: string, runAsync?: true, logErrorsOnly?: boolean): Promise<IExecResult>;
function runPrettier(files: string[], configPath: string, runAsync?: false, logErrorsOnly?: boolean): void;
function runPrettier(files: string[], configPath: string, runAsync?: boolean, logErrorsOnly?: boolean): Promise<IExecResult> | void {
  const cmd = [
    'node',
    prettierBin,
    '--config',
 ...
 *
 * @param projectPath - Path to the project root for which to run prettier
 */
export function runPrettierForProject(projectPath: string): Promise<IExecResult> {
  init();
 ...
 *
 * @param files - Staged files passed in by lint-staged
 * @param runAsync - Whether to run the command synchronously or asynchronously
 * @returns A promise if run asynchronously, or nothing if run synchronously
 */
export function runPrettierMultiProject(files: string[], runAsync?: boolean): void | Promise<void> {
  if (files.length === 0) {
