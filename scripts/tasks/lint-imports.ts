...
interface IImportErrorGroup {
  count: number;
  matches: { [filePath: string]: { importPath: string; alternative?: string }[] };
}

interface IImportErrors {
  /** (non-examples) Package-level imports from the current package */
  pathAbsolute: IImportErrorGroup;
  /** (all) No directory index imports */
  pathNotFile: IImportErrorGroup;
  /** (examples) No relative path imports */
  pathRelative: IImportErrorGroup;
  /** (examples) No "deep" imports: `pkg` and `pkg/lib/foo` are okay; `pkg/lib/foo/bar` is not */
  pathDeep: IImportErrorGroup;
  /** (examples) No direct imports from packages which OUFR re-exports */
  pathReExported: IImportErrorGroup;
  /** (examples) No `import *` due to editor limitations */
  importStar: IImportErrorGroup;
  /** (examples) No exporting multiple classes/consts from a single example */
  exportMulti: IImportErrorGroup;
  /** (examples) No default exports (due to editor limitations) */
  exportDefault: IImportErrorGroup;
}
...
export function lintImports() {
  ...
  function lintSource() {
    ...
    const importErrors: IImportErrors = {
    ...
    const importStats: IImportStats = {
   ...
   */
  function _getFiles(dir: string, extentionPattern: RegExp, fileList?: string[]) {
    fileList = fileList || [];
  ...
  function _evaluateFile(filePath: string, importErrors: IImportErrors, importStats: IImportStats, isExample: boolean) {
   ...
   */
  function _evaluateImport(filePath: string, importMatch: RegExpExecArray, importErrors: IImportErrors, isExample: boolean) {
  ...
  function _addError(errorGroup: IImportErrorGroup, relativePath: string, importPath: string, alternative?: string) {
    errorGroup.count++;
  ...
  function reportFilePathErrors(importErrors: IImportErrors) {
    const errorMessages: { [k in keyof IImportErrors]: string } = {
    ...
    for (const groupName of Object.keys(importErrors)) {
      const errorGroup: IImportErrorGroup = importErrors[groupName];
      if (errorGroup.count) {
        hasError = true;
