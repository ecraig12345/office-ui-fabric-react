...
import { INavPage, ISiteDefinition, currentFabricBreakpoint, jumpToAnchor } from '@uifabric/example-app-base/lib/index2';
...
export function createSite<TPlatforms extends string>(
  ...
  function _createRoutes(pages: INavPage<TPlatforms>[]): JSX.Element[] {
    ...
    const legacyComponentPages: INavPage<TPlatforms>[] = [];
    for (const page of allPages) {
      if (page.url === '#/controls/web') {
        // For the root web controls page, add an extra route for the old URL /components
        legacyComponentPages.push({ ...page, url: '#/components' });
      } else if (page.url.indexOf('/controls/web/') !== -1) {
        // For web control pages, add an extra route for the old URL under /components/
        legacyComponentPages.push({ ...page, url: page.url.replace('/controls/web/', '/components/') });
      }
    }
    allPages.push(...legacyComponentPages);
