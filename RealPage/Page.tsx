...
import { pascalize } from '@uifabric/example-app-base';
import { PageHeader2 } from '../PageHeader2/index';
import { Markdown } from '@uifabric/example-app-base/lib/components/Markdown';
import { ScrollBars } from '@uifabric/example-app-base/lib/components/ScrollBars';
import { SideRail, ISideRailLink } from '@uifabric/example-app-base/lib/components/SideRail';
import {
...
export class Page extends React.Component<IPageProps, IPageState> {
  ...
  private _getPageSections = (): IPageSectionProps[] => {
    ...
    feedback && sections.push({ renderAs: FeedbackSection, ...sectionProps, content: feedback });

    return sections;
  };

  private _getPageHeader2 = (): JSX.Element | undefined => {
    return <PageHeader2 pageTitle={this.props.title!} pageSubTitle={this.props.subTitle} />;
  };

  private _getSideRail = (): JSX.Element | undefined => {
    const { contactLinks, relatedLinks, showSideRail } = this.props;

    if (showSideRail) {
      let processedRelated: JSX.Element | ISideRailLink[] | undefined;
      if (typeof relatedLinks === 'string') {
        processedRelated = <Markdown>{relatedLinks}</Markdown>;
      } else {
        processedRelated = relatedLinks;
      }

      let processedContacts: JSX.Element | ISideRailLink[] | undefined;
      if (typeof contactLinks === 'string') {
        processedContacts = <Markdown>{contactLinks}</Markdown>;
      } else {
        processedContacts = contactLinks;
      }
