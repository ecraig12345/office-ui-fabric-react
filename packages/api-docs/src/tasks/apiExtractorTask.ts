...
// tslint:disable typedef interface-name no-function-expression no-any
...
export interface ApiExtractorOptions {
  ...
  /**
   * Callback after api-extractor is invoked. Files will already have been written, but if you'd
   * like to change them, `result` includes the appropriate paths.
...
interface ApiExtractorContext {
  ...
  /** Loaded config file */
  config: ApiExtractor.ExtractorConfig;
  /** Actual api-extractor module */
  apiExtractorModule: typeof ApiExtractor;
}
...
function initApiExtractor(options: ApiExtractorOptions): ApiExtractorContext | undefined {
  const apiExtractorModule = ApiExtractor;
...
function apiExtractorWrapper({
  ...
  extractorOptions,
  options
}: ApiExtractorContext): ApiExtractor.ExtractorResult | undefined {
