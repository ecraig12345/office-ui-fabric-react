...
import { getTableRowJson, getEnumTableRowJson } from './tableRowJson';
...
function createInterfacePageJson(collectedData: ICollectedData, interfaceItem: ApiInterface): ITableJson {
  const tableJson = getBasicTableJson(collectedData, interfaceItem, 'interface', interfaceItem.extendsTypes);

  tableJson.members = interfaceItem.members
    .map((member: ApiItem) => getTableRowJson(collectedData, member))
...
function createEnumPageJson(collectedData: ICollectedData, enumItem: ApiEnum): ITableJson {
  const tableJson = getBasicTableJson(collectedData, enumItem, 'enum');

  tableJson.members = enumItem.members.map((member: ApiEnumMember) => getEnumTableRowJson(collectedData, member));

  return tableJson;
}

function createClassPageJson(collectedData: ICollectedData, classItem: ApiClass): ITableJson {
  const tableJson = getBasicTableJson(collectedData, classItem, 'class', classItem.extendsType);
...
function createTypeAliasPageJson(collectedData: ICollectedData, typeAliasItem: ApiTypeAlias): ITableJson {
  const tableJson = getBasicTableJson(collectedData, typeAliasItem, 'typeAlias');
...
function getBasicTableJson(
  ...
  if (extendsArr && extendsArr.length) {
    ...
    for (const extendsType of extendsArr) {
      ...
      tableJson.extendsNames = tableJson.extendsNames || [];
      tableJson.extendsNames.push(extendsType.excerpt.text);
    }
  }
