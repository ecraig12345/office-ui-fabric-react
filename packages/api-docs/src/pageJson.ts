...
export function generatePageJson(
  ...
  for (const [pageName, pageData] of Object.entries(collectedData.pagesByName)) {
    result[pageName] = {
...
function initPageDataForItem(
  ...
  if (supportedApiItems.includes(apiItem.kind) && apiItem instanceof ApiDocumentedItem && apiItem.tsdocComment) {
    ...
    if (docCategoryTag) {
      ...
      let pageData = collectedData.pagesByName[pageName];

      if (pageData) {
        collectedData.pagesByApi[apiItem.displayName] = pageData;
      } else {
        pageData = collectedData.pagesByName[pageName] = collectedData.pagesByApi[apiItem.displayName] = {
          name: pageName,
          apiItems: [],
