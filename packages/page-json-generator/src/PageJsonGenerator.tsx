...
/**
 * Utilizing the api.json file, generates individual page json files
 */
export class PageJsonGenerator {
   ...
   */
  public generateJson(options: IPageJsonOptions): void {
    this.collectedData = new CollectedData();
    ...
    for (const pageName of options.pageNames) {
      this.collectedData.pageDataByPageName.set(pageName, new PageData(pageName));
    }

    this._collectPageData(apiEntryPoint);

    this._createPageJsonFiles(options);
  }
   ...
   */
  private _createPageJsonFiles(options: IPageJsonOptions): void {
    for (const pageName of options.pageNames) {
      const pageJsonPath: string = path.join(options.pageJsonFolderPath, pageName + '.page.json');
      console.log('Writing ' + pageJsonPath);

      const pageData: PageData = this.collectedData.pageDataByPageName.get(pageName)!;

      const pageJson: IPageJson = { tables: [] };
      for (const apiItem of pageData.apiItems) {
        switch (apiItem.kind) {
          case ApiItemKind.Interface: {
            pageJson.tables.push(this._createInterfacePageJson(apiItem as ApiInterface));
            break;
          }
          case ApiItemKind.Enum: {
            pageJson.tables.push(this._createEnumPageJson(apiItem as ApiEnum));
            break;
          }
          case ApiItemKind.Class: {
            pageJson.tables.push(this._createClassPageJson(apiItem as ApiClass));
            break;
          }
        }
      }
   ...
   */
  private _createInterfacePageJson(interfaceItem: ApiInterface): ITableJson {
    ...
    if (interfaceItem.tsdocComment) {
      tableJson.descriptionHtml += this._renderDocNodeWithoutInlineTag(interfaceItem.tsdocComment.summarySection);
    }
      ...
      switch (member.kind) {
        case ApiItemKind.PropertySignature: {
          ...
          if (apiPropertySignature.tsdocComment) {
            ...
            tableRowJson.descriptionHtml += this._renderDocNodeWithoutInlineTag(apiPropertySignature.tsdocComment.summarySection);
          }
        ...
        case ApiItemKind.MethodSignature: {
          ...
          if (apiMethodSignature.tsdocComment) {
            ...
            tableRowJson.descriptionHtml += this._renderDocNodeWithoutInlineTag(apiMethodSignature.tsdocComment.summarySection);
          }
          interfaceTableRowJson.push(tableRowJson);
          break;
        }
   ...
   */
  private _createEnumPageJson(enumItem: ApiEnum): ITableJson {
    ...
    if (enumItem.tsdocComment) {
      tableJson.descriptionHtml += this._renderDocNodeWithoutInlineTag(enumItem.tsdocComment.summarySection);
    }
      ...
      switch (member.kind) {
        case ApiItemKind.EnumMember: {
          ...
          if (apiEnumMember.tsdocComment) {
            tableRowJson.descriptionHtml += this._renderDocNodeWithoutInlineTag(apiEnumMember.tsdocComment.summarySection);
          }
   ...
   */
  private _createClassPageJson(classItem: ApiClass): ITableJson {
    ...
    if (classItem.tsdocComment) {
      tableJson.descriptionHtml += this._renderDocNodeWithoutInlineTag(classItem.tsdocComment.summarySection);
    }
      ...
      switch (member.kind) {
        case ApiItemKind.Property: {
          ...
          if (apiProperty.tsdocComment) {
            ...
            tableRowJson.descriptionHtml += this._renderDocNodeWithoutInlineTag(apiProperty.tsdocComment.summarySection);
          }
        ...
        case ApiItemKind.Method: {
          ...
          if (apiMethod.tsdocComment) {
            ...
            tableRowJson.descriptionHtml += this._renderDocNodeWithoutInlineTag(apiMethod.tsdocComment.summarySection);
          }
          classTableRowJson.push(tableRowJson);
          break;
        }
   ...
   */
  private _renderDocNodeWithoutInlineTag(docSection?: DocSection): string {
    let result = '';
    if (docSection) {
      ...
      for (const childNode of docSection.getChildNodes()) {
        result += this._renderDocNodeWithoutInlineTag(childNode as DocSection);
      }
    }
    return result;
  }
   ...
   */
  private _findInlineTagByName(tagName: string, docComment: DocComment): DocInlineTag | undefined {
   ...
   */
  private _collectPageData(apiItem: ApiItem): void {
      ...
      switch (apiItem.kind) {
        ...
        case ApiItemKind.Class: {
          ...
          if (apiItem.tsdocComment !== undefined) {
            const docCategoryTag: DocInlineTag | undefined = this._findInlineTagByName('@docCategory', apiItem.tsdocComment);
            ...
            if (docCategoryTag !== undefined) {
              ...
              const pageData: PageData | undefined = this.collectedData.pageDataByPageName.get(pageName);
    ...
    for (const memberApiItem of apiItem.members) {
      this._collectPageData(memberApiItem);
    }
  }
}
